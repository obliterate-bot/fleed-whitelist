...
walk(ast, {
  enter: (node, parent) => {
    if (node.type === NodeType.Identifier && node.name === 'MenuBar') {
      console.log(`[MenuBar] mangledName: ${node.mangledName}, isGlobal: ${node.isGlobal}, parent type: ${parent ? parent.type : 'none'}`);
    }
  }
});
