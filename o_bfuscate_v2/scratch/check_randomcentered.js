...
walk(ast, {
  enter: (node, parent) => {
    if (node.type === NodeType.Identifier && node.name === 'RandomCentered') {
      console.log(`[RandomCentered] mangledName: "${node.mangledName}", isGlobal: ${node.isGlobal}, parent: ${parent ? parent.type : 'none'}`);
    }
  }
});
