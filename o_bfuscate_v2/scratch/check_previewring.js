...
walk(ast, {
  enter: (node, parent) => {
    if (node.type === NodeType.Identifier && (node.name === 'PreviewRing' || node.name === 'HomeScore')) {
      console.log(`[${node.name}] mangledName: "${node.mangledName}", isGlobal: ${node.isGlobal}, parent: ${parent ? parent.type : 'none'}`);
    }
  }
});
