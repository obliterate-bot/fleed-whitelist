...
walk(ast, {
  enter: (node, parent) => {
    if (node.type === NodeType.Identifier && node.name === 'TerminalStatus') {
      console.log(`[TerminalStatus] mangledName: "${node.mangledName}", isGlobal: ${node.isGlobal}, parent: ${parent ? parent.type : 'none'}`);
    }
  }
});
