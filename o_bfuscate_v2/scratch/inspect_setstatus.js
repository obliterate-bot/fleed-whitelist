...
walk(ast, {
  enter: (node, parent) => {
    if (node.type === NodeType.Identifier && node.name === 'SetTerminalStatus') {
      console.log(`[SetTerminalStatus] mangledName: "${node.mangledName}", isGlobal: ${node.isGlobal}, parent: ${parent ? parent.type : 'none'}`);
    }
  }
});
