...
walk(ast, {
  enter: (node, parent) => {
    if (node.type === NodeType.Identifier && (node.name === 'Compat' || node.name === 'CloseMenus' || node.name === 'TerminalStroke')) {
      console.log(`[${node.name}] mangledName: ${node.mangledName}, isGlobal: ${node.isGlobal}, parent type: ${parent ? parent.type : 'none'}`);
    }
  }
});
