...
walk(ast, {
  enter: (node, parent) => {
    if (node.type === NodeType.Identifier && node.name === 'AUTO_BODY_BIND_NAME') {
      console.log(`[AUTO_BODY_BIND_NAME] mangledName: "${node.mangledName}", isGlobal: ${node.isGlobal}, parent: ${parent ? parent.type : 'none'}`);
    }
  }
});
